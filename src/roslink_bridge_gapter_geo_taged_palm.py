...
from messages.RoslinkGeoTagedImage import RoslinkGeoTagedImage
...
# <<<<<<< HEAD:src/roslink_bridge_gapter_geo_taged_dji.py
...
# =======
i=0
# >>>>>>> e5aa4f42615e91fd2c06e966a2288109644ddca8:src/roslink_bridge_gapter_geo_taged.py
class ROSLinkBridgeGapter:
    ...
    def init_servers():
        ...
        # ROSLinkBridgeGapter.cap = cv2.VideoCapture("/home/ros/exam_catkin_ws/src/roslink/src/DJI_0001.mov")
        global i
        ROSLinkBridgeGapter.cap = cv2.VideoCapture(0)
        ROSLinkStateVariables.ws_timer = 1
        ROSLinkBridgeGapter.WSconnected=False
        ROSLinkBridgeGapter.ws = None;

        
        t = threading.Thread(target=ROSLinkBridgeGapter.websocketThread)
    ...
    @staticmethod
    def websocketThread():
        if(not ROSLinkBridgeGapter.WSconnected):
            # this for public network
            WS_url = "ws://"+ ROSLinkBridgeGapter.gcs_server_ip+":9090/websockets/roslink/user/"+str(ROSLinkStateVariables.owner_id) +"/robot/"+ROSLinkStateVariables.key+"/robot" 
            # WS_url = "ws://"+ ROSLinkBridgeGapter.gcs_server_ip+":9090/";
            ROSLinkBridgeGapter.ws = websocket.WebSocketApp(WS_url,
                                on_message = ROSLinkBridgeGapter.on_message,
                                on_error = ROSLinkBridgeGapter.on_error,
                                on_close = ROSLinkBridgeGapter.on_close,
                                )
            ROSLinkBridgeGapter.ws.on_open = ROSLinkBridgeGapter.on_open
            
            # ROSLinkBridgeGapter.WSconnected=True;
            ROSLinkBridgeGapter.ws.run_forever()
    ...
    def on_close(ws):   
        ...
        ROSLinkStateVariables.ws_timer+=1;
        ROSLinkBridgeGapter.handel_websocket_closing();
    ...
    def on_open(ws):
        ...
        time.sleep(2) # to make sure no thread is sending now
        ROSLinkStateVariables.WSconnected =True;


    @staticmethod
    def on_message(ws, message):
        ROSLinkBridgeGapter.process_roslink_command_message(message)
    ...
    def init_params():   
        ...
        ROSLinkStateVariables.sequence_numbers= {0:0}
        ROSLinkStateVariables.sequence_numbers[MESSAGE_TYPE.ROSLINK_MESSAGE_HEARTBEAT]=0
        ROSLinkStateVariables.sequence_numbers[MESSAGE_TYPE.ROSLINK_MESSAGE_ROBOT_STATUS]=0
        ROSLinkStateVariables.sequence_numbers[MESSAGE_TYPE.ROSLINK_MESSAGE_GLOBAL_MOTION]=0
        ROSLinkStateVariables.sequence_numbers[MESSAGE_TYPE.ROSLINK_MESSAGE_ROSLINK_GEO_TAGED_IMAGE]=0
        ROSLinkStateVariables.sequence_numbers[MESSAGE_TYPE.ROSLINK_MESSAGE_GPS_RAW_INFO]=0
        ROSLinkStateVariables.sequence_numbers[MESSAGE_TYPE.ROSLINK_MESSAGE_ROSLINK_IMAGE]=0
        # import os
        # dirpath = os.getcwd()
        # print("current directory is : " + dirpath)
        SRT = open("/home/ros/exam_catkin_ws/src/roslink/DJI_0001.SRT", "r")
        ROSLinkBridgeGapter.SRTLines = SRT.readlines()
        ROSLinkBridgeGapter.SRTLines_counter = 3
        # ROSLinkBridgeGapter.bridge = CvBridge()
        ROSLinkStateVariables.imcounter=0;
        ROSLinkStateVariables.prefix="/home/ros/exam_catkin_ws/src/roslink/healthy/"
        ROSLinkStateVariables.maxframs= 293
    #start ROS publishers 
    @staticmethod
    ...
    def start_ros_subscribers():
        ...
        # rospy.Subscriber("/camera/rgb/image_raw", Image, ROSLinkBridgeGapter.frontImageCallback)
    ...
    def create_roslink_message_threads(): 
        ROSLinkBridgeGapter.heatrbeat_message_thread = ROSLinkMessageThread(ROSLinkBridgeGapter.client_socket, ROSLinkBridgeGapter.ws, ROSLinkBridgeGapter.server_address, MESSAGE_TYPE.ROSLINK_MESSAGE_HEARTBEAT, "heatrbeat_message_thread", ROSLinkBridgeGapter.heartbeat_msg_rate)
        ROSLinkBridgeGapter.robot_status_thread = ROSLinkMessageThread(ROSLinkBridgeGapter.client_socket, ROSLinkBridgeGapter.ws, ROSLinkBridgeGapter.server_address, MESSAGE_TYPE.ROSLINK_MESSAGE_ROBOT_STATUS, "robot_status_message_thread", ROSLinkBridgeGapter.robot_status_msg_rate)
        ROSLinkBridgeGapter.global_motion_thread = ROSLinkMessageThread(ROSLinkBridgeGapter.client_socket, ROSLinkBridgeGapter.ws, ROSLinkBridgeGapter.server_address, MESSAGE_TYPE.ROSLINK_MESSAGE_GLOBAL_MOTION, "global_motion_thread", ROSLinkBridgeGapter.global_motion_msg_rate)
        ROSLinkBridgeGapter.gps_raw_info_thread = ROSLinkMessageThread(ROSLinkBridgeGapter.client_socket,ROSLinkBridgeGapter.ws,  ROSLinkBridgeGapter.server_address, MESSAGE_TYPE.ROSLINK_MESSAGE_GPS_RAW_INFO, "gps_raw_info_thread", ROSLinkBridgeGapter.gps_raw_info_msg_rate)
        ROSLinkBridgeGapter.roslink_image_thread = ROSLinkMessageThread(ROSLinkBridgeGapter.client_socket, ROSLinkBridgeGapter.ws, ROSLinkBridgeGapter.server_address, MESSAGE_TYPE.ROSLINK_MESSAGE_ROSLINK_GEO_TAGED_IMAGE, "roslink_image_thread", ROSLinkBridgeGapter.ROSLing_Image_msg_rate )
    ...
    def static_build_ROSLink_image_message():
        ...
        # ROSLinkBridgeGapter.cap.read()
# <<<<<<< HEAD:src/roslink_bridge_gapter_geo_taged_dji.py
        # ret, frame = ROSLinkBridgeGapter.cap.read()
        # frame = cv2.resize(frame,(400,300))
        # im_name = "/home/ros/exam_catkin_ws/src/roslink/DJI_00"+str(ROSLinkStateVariables.imcounter)+".JPG"
        t1 = time.time();

        # im_name = ROSLinkStateVariables.prefix+str(ROSLinkStateVariables.imcounter)+".jpg"
        # # if(ROSLinkStateVariables.imcounter>99):
        # #     im_name = "/home/ros/exam_catkin_ws/src/roslink/date/DJI_0"+str(ROSLinkStateVariables.imcounter)+".jpg"
        # print(im_name)
        # ROSLinkStateVariables.imcounter = ROSLinkStateVariables.imcounter+3
        # if(ROSLinkStateVariables.imcounter>ROSLinkStateVariables.maxframs):
        #     ROSLinkStateVariables.prefix="/home/ros/exam_catkin_ws/src/roslink/unhealthy/"
        #     ROSLinkStateVariables.imcounter=0
        #     ROSLinkStateVariables.maxframs=2575
            
        # # if(ROSLinkStateVariables.imcounter>06
        ...
        # metaData = {}
        # imgFile = PIL.Image.open(im_name)
        ...
        #         if tagname=="GPSInfo":
        #             temp= str(value).split("2:")[1].replace('(', '').replace(')', '')
        #             lats = temp.split(",")
        #             ROSLinkStateVariables.lat = int(lats[0]) + (int(lats[2])/60.0) + (int(lats[4])/36000000.0)
        #             lons= temp.split("4:")[1].split(",")
        #             ROSLinkStateVariables.lon = int(lons[0]) + ((int(lons[2])/60.0) + (int(lons[4])/36000000.0))
        #             # print tagname, lat, lon

        #             break;



        # # encoded = cv2.imencode('.jpg',encimg)[1]
        # data = base64.encodestring(encimg)
        # data = ROSLinkStateVariables.ROSLinkImageData
        # t2 = time.time();
        ...
        data = ROSLinkStateVariables.ROSLinkImageData
        message_header = ROSLinkBridgeGapter.static_build_roslink_header_message(MESSAGE_TYPE.ROSLINK_MESSAGE_ROSLINK_GEO_TAGED_IMAGE) 

        Roslink_geo_taged_mage = RoslinkGeoTagedImage(message_header, ROSLinkStateVariables.owner_id ,ROSLinkStateVariables.key,data,ROSLinkStateVariables.ROSLinkImageHeight,ROSLinkStateVariables.ROSLinkImageWidth,
        ROSLinkStateVariables.lat, ROSLinkStateVariables.lon, ROSLinkStateVariables.alt, 0,0,0)
        print(len(data))

        return Roslink_geo_taged_mage.__dict__ 
            ...
            try:
                if (self.roslink_message_type == MESSAGE_TYPE.ROSLINK_MESSAGE_HEARTBEAT):
                    if (ROSLinkStateVariables.WSconnected):
                        ...
                        self.WS.send(json.dumps(ROSLinkBridgeGapter.static_build_heartbeat_message()))
                    else:
                        self.send(self.socket, json.dumps(ROSLinkBridgeGapter.static_build_heartbeat_message()))
                        self.WS = ROSLinkBridgeGapter.ws;

                elif (self.roslink_message_type == MESSAGE_TYPE.ROSLINK_MESSAGE_ROBOT_STATUS):
                    if (ROSLinkStateVariables.WSconnected):
                        # print("status by ws")
                        self.WS.send(json.dumps(ROSLinkBridgeGapter.static_build_robot_status_message()))
                    else:
                    ...
                    if (ROSLinkStateVariables.WSconnected):
                        self.WS.send(json.dumps(ROSLinkBridgeGapter.static_build_global_motion_message()))
                        # print("global_motion_message sent --------------")
                    else:
                    ...
                    if (ROSLinkStateVariables.WSconnected):
                        self.WS.send(json.dumps(ROSLinkBridgeGapter.static_build_gps_raw_info_message()))
                    else:
                    ...
                    if (ROSLinkStateVariables.WSconnected):
                        self.WS.send(json.dumps(ROSLinkBridgeGapter.static_build_ROSLink_image_message()))
                        ...
                        self.WS = ROSLinkBridgeGapter.ws;
                elif (self.roslink_message_type == MESSAGE_TYPE.ROSLINK_MESSAGE_ROSLINK_GEO_TAGED_IMAGE):
                    if (ROSLinkStateVariables.WSconnected):
                        self.WS.send(json.dumps(ROSLinkBridgeGapter.static_build_ROSLink_image_message()))
                        self.WS.send(json.dumps(ROSLinkBridgeGapter.static_build_ROSLink_image_message()))
                        # print ('ROSLink_geotagged_image sent ws')
                        # self.WS.send(json.dumps(ROSLinkBridgeGapter.static_build_ROSLink_image_message()))
                        print ('ROSLink_geotagged_image sent ws')
# >>>>>>> e5aa4f42615e91fd2c06e966a2288109644ddca8:src/roslink_bridge_gapter_geo_taged.py
                    else:
